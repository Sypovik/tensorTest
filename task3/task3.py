...
class VersionAnalyzer:
    ...
    def _load_config(self) -> Dict[str, str]:
        ...
        try:
            ...
            for key, value in config.items():
                if not isinstance(value, str) or not pattern.fullmatch(value) or value.count("*") > 1:
    ...
    @staticmethod
    def _generate_version_variants(template: str) -> List[str]:
        """Генерирует 2 варианта версии для шаблона"""
        return [template.replace('*', str(i)) for i in (0, 1)]  
    ...
    def _generate_all_versions(self) -> List[str]:
        ...
        return [
            ...
            for version in self._generate_version_variants(template)
        ]
...
def main():
    ...
    parser.add_argument('-v', '--version', required=True, help='Целевая версия')
